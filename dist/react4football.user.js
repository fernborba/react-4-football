...
// @require      https://raw.githubusercontent.com/fernborba/react-4-football/refs/heads/main/dist/index4.js?v=v11.0.92
// ==/UserScript==
...
`;
  ...
  const EXPECTED_BUNDLE_VERSION = "v11.0.92";
  const startupState = {
    failed: false,
    reason: null,
  };
    ...
    try {
      ...
      if (stored) {
        ...
        return {
          ...
          priceCheckOnUnassignedEnabled: parsed.priceCheckOnUnassignedEnabled !== false,
        };
      }
    } catch (e) {
      console.error("[R4F] Error reading exclude config:", e);
    }
    return { priceCheckEnabled: true, priceCheckOnUnassignedEnabled: true };
  }
  ...
  function formatPriceLabel(cached) {
    ...
    if (cached.status === "untradable") return "Untradeable";
    if (cached.status === "not-found") return "Extinct";
    if (cached.status === "error") return "Error";
    if (cached.status === "priced") {
      const n = cached.lowestBin != null ? cached.lowestBin : cached.averageBin;
      return n != null ? formatPriceK(n) : "—";
    }
    return "";
  }

  /** Transfer-market / cache id for players (matches React PricedPlayerCard: assetId as definitionId). */
  function getPlayerPriceDefinitionId(item) {
    if (!item || !item.isPlayer || !item.isPlayer()) return null;
    if (typeof item.assetId === "number" && item.assetId > 0) return item.assetId;
    if (typeof item.definitionId === "number" && item.definitionId > 0) return item.definitionId;
    return null;
  }
  ...
  async function injectPlayerItemPrice(rootEl, item) {
    ...
    const definitionId = getPlayerPriceDefinitionId(item);
    ...
    let label = rootEl.querySelector(".r4f-price-label");
    if (!label) {
      label = document.createElement("span");
      label.className = "r4f-price-label";
      rootEl.appendChild(label);
    }
    label.textContent = "…";

    try {
      const cached = await window.R4F.priceCache.get(definitionId);
      if (cached) {
        label.textContent = formatPriceLabel(cached);
        return;
      }
    ...
    if (typeof UTPlayerItemView !== "undefined") {
      ...
      UTPlayerItemView.prototype.renderItem = function (item, t) {
        ...
        if (item && rootEl) {
          ...
          void injectPlayerItemPrice(rootEl, item);
        }
  ...
  function initPreviewPackValuation() {
    let capturedPreview = { packId: null, itemList: null };

    function previewPackIdFromUrl(url) {
      const href = url instanceof URL ? url.href : String(url || "");
      const match = href.match(/\/preview\/pack\/(\d+)\/items(?:[/?#]|$)/);
      return match ? Number(match[1]) : null;
    }

    function previewPackIdFromFetchInput(input) {
      if (typeof input === "string" || input instanceof URL) {
        return previewPackIdFromUrl(input);
      }
      return previewPackIdFromUrl(input?.url ?? "");
    }

    function rememberPreviewResponse(packId, data) {
      if (capturedPreview.packId !== packId || !Array.isArray(data?.itemList)) return;
      capturedPreview = { packId, itemList: data.itemList };
      console.log("[R4F] Captured preview pack items", {
        packId,
    ...
    window.fetch = function (input) {
      const packId = previewPackIdFromFetchInput(input);
      if (packId != null) capturedPreview = { packId, itemList: null };
      const promise = originalFetch.apply(this, arguments);
      if (packId != null) {
        promise.then((response) => {
          if (!response?.ok) return;
          response.clone().json()
            .then((data) => rememberPreviewResponse(packId, data))
            .catch(() => {});
        }).catch(() => {});
      }
      return promise;
    ...
    XMLHttpRequest.prototype.open = function (method, url) {
      const packId = previewPackIdFromUrl(url);
      if (packId != null) {
        capturedPreview = { packId, itemList: null };
        this.addEventListener("load", function () {
          if (this.status < 200 || this.status >= 300) return;
          try {
            rememberPreviewResponse(
              packId,
    ...
    function sourceForModal(modal) {
      return {
        get packId() {
          const modalPackId = Number(
            modal.dataset.packId || modal.querySelector("[data-pack-id]")?.dataset.packId
          );
          return Number.isInteger(modalPackId) && modalPackId > 0
            ? modalPackId
            : capturedPreview.packId || undefined;
        },
        get itemList() {
          return this.packId === capturedPreview.packId && Array.isArray(capturedPreview.itemList)
            ? capturedPreview.itemList
            : undefined;
        },
      };
    }
    ...
    function attachOpenModals() {
      if (typeof window.R4F?.previewPackUi?.attach !== "function") return;
      document.querySelectorAll(".ut-store-reveal-modal-list-view").forEach((modal) => {
        window.R4F.previewPackUi.attach(modal, sourceForModal(modal));
      });
    }

    const observer = new MutationObserver(attachOpenModals);
    observer.observe(document.body, { childList: true, subtree: true });
    attachOpenModals();
    console.log("[R4F] Preview pack valuation observer installed");
  }
...
`;
  ...
  const previewPackStyleSheet = document.createElement("style");
  previewPackStyleSheet.innerText = `
.r4f-preview-value-host{position:relative}
.r4f-preview-value{position:absolute;left:50%;bottom:3px;z-index:3;transform:translateX(-50%);padding:2px 5px;border:1px solid #6fd7ff;border-radius:5px;background:rgba(21,27,35,.94);color:#fff;font-size:11px;font-weight:700;line-height:1.25;pointer-events:none;white-space:nowrap}
.r4f-preview-value.is-discard{border-color:#d6a64b;color:#ffd980}
.r4f-preview-value.is-error{border-color:#e35d6a;color:#ff9ca6}
.r4f-preview-pack-summary{flex:1 0 100%;order:10;box-sizing:border-box;padding:6px 8px;color:#f4f4f6;font-size:12px;font-weight:700;text-align:center}
.r4f-preview-pack-summary.is-positive{color:#65d889}
.r4f-preview-pack-summary.is-negative,.r4f-preview-pack-summary-error{color:#ff7f88}
.r4f-value-pack-btn:disabled{cursor:wait;opacity:.65}
`;
  document.head.appendChild(previewPackStyleSheet);

  // Intercept EA's fetch/XHR calls to track active SBC challenge + set IDs
  initSbcRequestInterceptor();
  initPreviewPackValuation();
